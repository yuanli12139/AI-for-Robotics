...
def smooth(path, weight_data=0.1, weight_smooth=0.1, tolerance=0.00001):
    ...
    while change >= tolerance:
        ...
        for i in range(0, len(path)):
            ...
            if i == 0:
                for j in range(len(path[0])):
                    aux = weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[i+1][j] + newpath[len(path)-1][j] - 2 * newpath[i][j])
                    newpath[i][j] += aux
                    change += abs(aux)

            elif i == len(path) - 1:
                for j in range(len(path[0])):
                    aux = weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[0][j] + newpath[i-1][j] - 2 * newpath[i][j])
                    newpath[i][j] += aux
                    change += abs(aux)

            else:
                for j in range(len(path[0])):
                    aux = weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[i+1][j] + newpath[i-1][j] - 2 * newpath[i][j])
                    newpath[i][j] += aux
                    change += abs(aux)

        # A more smarter solution from Thrun:
        #     for j in range(len(path[0])):
        #         aux = weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[(i+1)%len(path)][j] + newpath[(i-1)%len(path)][j] - 2 * newpath[i][j])
        #         newpath[i][j] += aux
        #         change += abs(aux)
